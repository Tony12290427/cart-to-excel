...
## 直接存到 Google Drive（可选，需一次性设置）

除了下载到本地，还可以把导出的表格**直接存进指定的 Drive 文件夹**。纯静态网页不能凭空写
Drive，所以中间需要一层"替你签名"的服务 —— 用 Google Apps Script 最省事：**不用建 Google Cloud
项目、不用 OAuth 客户端、不用审核、同学也不用授权弹窗**。

### 一次性设置（约 5 分钟，必须你本人的 Google 账号操作）

1. 打开 https://script.google.com → **新建项目**
2. 把 `tools/drive-upload.gs` 的内容整份粘进 `Code.gs`
   （里面的 `FOLDER_ID` 已经填好你的文件夹了，不用改）
3. 把 `TOKEN` 改成任意一串随机字符，**并确保和网页里的一致**
4. **部署 → 新建部署 → 网页应用**：
   - 执行身份：**我**
   - 谁可以访问：**任何人**
   - 授权时同意即可（这是你自己的脚本）
5. 复制得到的 `/exec` 网址，填进 `index.html` 顶部的 `DRIVE_UPLOAD_URL`

设置完成后，导出按钮旁边会多出一个 **Save to Drive**，点一下文件就进文件夹，
成功后会显示文件名和"open file"链接。

### 加固：为什么还要校验文件名和内容

网页在公开仓库里，所以**网址和 TOKEN 都是公开可读的**。只靠 TOKEN 的话，拿到网址的人可以把这个
接口当"免费网盘"用 —— 传任意文件、再借 `drive.google.com` 的域名信誉分发出去（这叫信誉洗白，
是真实存在的攻击手法）。所以脚本还做了三道校验：

| 校验 | 拦住什么 |
|---|---|
| 文件名必须匹配 `^\d{8}( .+)? order form\.xlsx$` | `payload.exe`、`shell.php` 这类任意文件 |
| 内容必须是 ZIP 且内含 `xl/workbook.xml` | 伪装成 xlsx 的其它东西；xlsx 本身装不了 VBA 宏（那是 .xlsm），所以通过校验的一定是普通表格 |
| 每天最多 50 次 | 被当成批量上传接口刷 |

被拒时会在网页上显示原因（如 `unexpected file name: …`），不会静默失败。

### 这个接口能做什么、不能做什么

| 能做 | 不能做 |
|---|---|
| 往文件夹里**新建**文件 | ❌ 列不出文件夹里有什么（脚本没有任何列举接口） |
| 名字冲突时**加序号**（`… (2).xlsx`） | ❌ 删不掉任意文件 —— 只对"上传时同名"的文件有 trash 能力，而且默认关闭 |
| 返回"我这次传的文件"的名字和链接 | ❌ 读不到任何已有文件的内容，也动不了文件夹本身 |

**关于删除**：脚本里唯一的删除能力是"同名替换"，默认 **关闭**（`REPLACE_SAME_NAME = false`）。
开着它的话，任何拿到公开 URL 的人只要猜中文件名，就能把队友交的那份挤进回收站 —— 这是这个端点
唯一能造成损失的方式，所以默认关掉。关掉后接口**只增不减**：最坏情况是多出垃圾文件，不会丢东西。
（真要开，把 `REPLACE_SAME_NAME` 改成 `true` 即可，但要清楚上面这个代价。）

### 几个要知道的点

- **上传的文件归"部署者"所有**（也就是你），占你的 Drive 配额；文件夹分享给社团即可
- **重复导出会并存**：同一同学导出两次会得到 `… order form.xlsx` 和 `… order form (2).xlsx` 两份，
  需要合并时手动挑一份删掉即可（比"自动删旧的"安全）
- **`TOKEN` 只是挡扫描器的**，不是真安全：网页是公开的，看源码就能拿到它。
  要换就两边同时改
- 没配置 `DRIVE_UPLOAD_URL` 时，这个按钮**完全不显示**，功能等于不存在
- 排查：脚本编辑器里先跑一次 `testWrite()`，能写进文件说明文件夹 ID 和权限都对
